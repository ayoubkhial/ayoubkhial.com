...
export default function AdaptiveImage({ src, alt, width, height, priority = false, adaptive = true }: Props) {
  ...
  const {
    props: { srcSet: dark }
  } = getImgProps({ ...common, src: adaptive ? `${filename}_dark.${extension}` : src });
  const {
  ...
  return (
    <picture>
      ...
      <img
        className="border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-900 my-5 rounded-xl border border-dashed p-4"
        {...rest}
      />
    </picture>
  );
